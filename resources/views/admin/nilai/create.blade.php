        ...
        <div class="col-lg-12">
            <div class="main-card mb-3 card">
                <div class="card-body">
                    ...
                    <form action="{{ route('nilai.store') }}" class="needs-validation" novalidate method="POST">
                        @csrf
                        <div class="form-row">
                            <div class="col-md-2 mb-3">
                                <label for="kd_pelajaran">Kelas</label>
                                <select type="select" name="kd_pelajaran" id="kd_pelajaran" name="customSelect" class="custom-select" required>
                                    <option value="">Pilih</option>
                                    @foreach ($pelajaran as $item)
                                    <option value="{{ old('kd_pelajaran') ?? $item->kd_pelajaran }}">{{ $item->nm_pelajaran }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-5 mb-3">
                                ...
                                <select type="select" name="NIS" id="NIS" name="customSelect" class="custom-select" required>
                                    ...
                                    <option value="{{ old('NIS') ?? $item->NIS }}">{{ $item->NIS }} | {{ $item->nm_siswa }}</option>
                                    @endforeach
                                </select>
                                <div class="invalid-tooltip">
                                    Tidak Boleh Kosong
