...
<div class="app-main__inner">
    ...
    <div class="row">
        <div class="col-lg-12">
            <a href="{{ route('kelasDetail.create') }}"class="mb-2 mr-2 btn-transition btn btn-outline-secondary">Tambah Data @yield('judul')</a>
            <div class="main-card mb-3 card">
                <div class="card-body table-responsive"><h5 class="card-title">Data @yield('judul')</h5>
                    <table class="mb-0 table table-bordered">
                        ...
                        <tbody>
                        @foreach ($kelasDetail as $item)
                        <tr>
                            <th scope="row">{{$loop->iteration}}</th>
                            <td>{{ $item->kd_kelas }}</td>
                            <td>{{ $item->NIS }} - {{ $item->siswa->nm_siswa }}</td>
                            <td>{{ $item->thn }}</td>
                            <td>{{ $item->semester }}</td>
                            <td>
                                <div class="text-left">
                                    <div role="group" class="btn-group-sm btn-group btn-group-toggle" data-toggle="buttons">
                                        <button id="edit" href="kelasDetail/{{ $item->id }}/edit" type="button" class="btn btn-alternate">Edit
                                        </button>
                                        <button id="delete" href="kelasDetail/{{ $item->id }}" class="btn btn-danger">Hapus</button>
                                    </div>
                                </div>
                            </td>
                        </tr>
